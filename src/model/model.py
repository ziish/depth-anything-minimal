...
from model.blocks import FeatureFusionBlock, _make_scratch
from model.transform import NormalizeImage, PrepareForNet, Resize
...
class DPTHead(nn.Module):
    ...
    def forward(self, out_features, patch_h, patch_w):
        ...
        path_4 = self.scratch.refinenet4(layer_4_rn, size=layer_3_rn.shape[2:])  # type: ignore
        path_3 = self.scratch.refinenet3(path_4, layer_3_rn, size=layer_2_rn.shape[2:])  # type: ignore
        path_2 = self.scratch.refinenet2(path_3, layer_2_rn, size=layer_1_rn.shape[2:])  # type: ignore
        path_1 = self.scratch.refinenet1(path_2, layer_1_rn)  # type: ignore

        out = self.scratch.output_conv1(path_1)  # type: ignore
        out = F.interpolate(out, (int(patch_h * 14), int(patch_w * 14)), mode="bilinear", align_corners=True)
        out = self.scratch.output_conv2(out)  # type: ignore
